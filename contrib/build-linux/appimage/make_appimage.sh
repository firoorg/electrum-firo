...
APPIMAGE="$DISTDIR/electrum-firo-$VERSION-x86_64.AppImage"
...
cp -f "$PROJECT_ROOT/electrum_firo/libsecp256k1.so.0" "$APPDIR/usr/lib/libsecp256k1.so.0" || fail "Could not copy libsecp to its destination"
