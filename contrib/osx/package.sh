...
if [ -z "$1" ]; then
    echo "Usage: $0 Electrum-Firo.app"
    exit -127
fi
...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-firo-macos
	cd /tmp/electrum-firo-macos
...
rm -rf /tmp/electrum-firo-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-firo-macos/image/
cp -r $1 /tmp/electrum-firo-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum_Firo_uncompressed.dmg \
    /tmp/electrum-firo-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum_Firo_uncompressed.dmg electrum-firo-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum_Firo_uncompressed.dmg

echo "Done."
sha256sum electrum-firo-$VERSION.dmg
